...
<template>
    <ContactSuccess v-if="isSuccess" @reset="reset" />

    <form v-else id="contact-form" @submit.prevent="submit" class="flex flex-col gap-3.5">
        ...
        >
            <span>{{ isLoading ? t('footer.sending', 'Enviando…') : t('footer.send') }}</span>
            <NavigationIcon
                v-if="!isLoading"
